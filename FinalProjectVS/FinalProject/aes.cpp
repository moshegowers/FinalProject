...
aes::aes(std::string & KeyWord){
	std::vector<unsigned char> v(KeyWord.begin(), KeyWord.end());
    set_key(v);
}
...
std::vector<unsigned char> aes::get_round_key_matrix()
{
	std::vector<unsigned char> res;
	std::vector< std::vector<unsigned char> > key = _KeyRoundMatrix;
	for (std::vector< std::vector<unsigned char> >::iterator it = key.begin(); it != key.end(); ++it)
	{
...
{
	std::vector<unsigned char> x = get_round_key_matrix();
	std::vector<unsigned char> res = aes_base::encrypt(std::vector<unsigned char>(plaintext.begin(), plaintext.end()), x);
	return std::string(res.begin(), res.end());
}

std::string aes::Decrypt(std::string plaintext)
{
	return std::string();
}
